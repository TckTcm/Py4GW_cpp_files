...
namespace {
    ...
    bool IsMoveItemPacketSafe(const MoveItem_UIMessage& packet) {
        const auto item = Items::GetItemById(packet.item_id);
        const auto bag = Items::GetBagByIndex(packet.bag_index);
        if (!(item && bag && bag->items.valid()))
            return false;
        if (!CanInteractWithItem(item))
            return false;
        if (!IsItemStillInSourceBag(item))
            return false;
        if (!packet.slot || bag->items.size() < packet.slot)
            return false;
        if (!packet.quantity || packet.quantity > item->quantity)
            return false;
        return true;
    }

    bool IsItemUpdatePacketSafe(const UI::UIPacket::kItemUpdated& packet) {
        const auto item = Items::GetItemById(packet.item_id);
        if (!item)
            return true;
        return IsItemStillInSourceBag(item);
    }
    ...
    void OnMoveItem_UIMessage(GW::HookStatus* status, UI::UIMessage message_id, void* wparam, void*) {
        ...
        if (!status->blocked && !IsMoveItemPacketSafe(*packet)) {
            status->blocked = true;
            GWCA_INFO(
                "[Item Module] Blocked stale MoveItem item=%u quantity=%u bag=%u slot=%u",
                packet->item_id,
                packet->quantity,
                packet->bag_index,
                packet->slot);
    ...
    void OnItemUpdated_UIMessage(GW::HookStatus* status, UI::UIMessage message_id, void* wparam, void*) {
        ...
        if (!IsItemUpdatePacketSafe(*packet) || IsInventoryEquipmentSelectionStale()) {
            status->blocked = true;
            GWCA_INFO(
    ...
    void Init() {
        ...
        GWCA_INFO("[SCAN] ValidateUpgrade_Func = %p", ValidateUpgrade_Func);
        GWCA_INFO("[SCAN] OrderUpgradeBegin_Func = %p", OrderUpgradeBegin_Func);
        GWCA_INFO("[SCAN] OrderUpgradeAdd_Func = %p", OrderUpgradeAdd_Func);
        GWCA_INFO("[SCAN] OrderUpgradeEnd_Func = %p", OrderUpgradeEnd_Func);
        GWCA_INFO("[SCAN] GetInventoryIDFromAgent_Func = %p", GetInventoryIDFromAgent_Func);
        GWCA_INFO("[SCAN] GetEquippedItemID_Func = %p", GetEquippedItemID_Func);
        GWCA_INFO("[SCAN] InventoryTableFind_Func = %p", InventoryTableFind_Func);
